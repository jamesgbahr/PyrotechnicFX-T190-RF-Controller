...
set "PFX_WORKSPACE=%USERPROFILE%\.pfxpio\rx"
...
echo Installing dependencies and building receiver firmware...
"%PIO%" run -e t190_receiver
if errorlevel 1 goto :fail

echo.
echo BUILD COMPLETE.
echo Firmware:
echo   %PFX_WORKSPACE%\build\t190_receiver\firmware.bin
pause
exit /b 0
